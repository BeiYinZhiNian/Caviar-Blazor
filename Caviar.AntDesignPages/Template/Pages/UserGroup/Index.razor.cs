...
namespace Caviar.AntDesignPages.Pages.UserGroup
{
    ...
    public partial class Index
    {
        ...
        /// 数据源
        /// </summary>
        List<ViewUserGroup> DataSource { get; set; } = new List<ViewUserGroup>();
        /// <summary>
        /// 总计
        /// </summary>
        ...
        async Task GetPages(int pageIndex = 1, int pageSize = 10, bool isOrder = true)
        {
            ...
            var result = await Http.GetJson<PageData<ViewUserGroup>>($"{url}?pageIndex={pageIndex}&pageSize={pageSize}&isOrder={isOrder}");
        ...
        {
            var result = await Http.GetJson<List<ViewModelFields>>("UserGroup/GetFields");
            if (result.Status != 200) return;
        ...
        async void Delete(string url,ViewUserGroup data)
        {
            //删除单条
        ...
        partial void PratialRowCallback(ref bool isContinue, RowCallbackData<ViewUserGroup> row);
        /// <summary>
        /// 翻页按钮回调
        /// </summary>
        /// <param name="isContinue"></param>
        ...
        void RowCallback(RowCallbackData<ViewUserGroup> row)
        {
            bool isContinue = true;
