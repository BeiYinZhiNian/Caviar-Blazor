...
namespace Caviar.AntDesignPages.Pages.User
{
    ...
    public partial class Index
    {
        ...
        /// 数据源
        /// </summary>
        List<ViewUser> DataSource { get; set; } = new List<ViewUser>();
        /// <summary>
        /// 总计
        /// </summary>
        ...
        async Task GetPages(int pageIndex = 1, int pageSize = 10, bool isOrder = true)
        {
            ...
            var result = await Http.GetJson<PageData<ViewUser>>($"{url}?pageIndex={pageIndex}&pageSize={pageSize}&isOrder={isOrder}");
        ...
        {
            var result = await Http.GetJson<List<ViewModelFields>>("User/GetFields");
            if (result.Status != 200) return;
        ...
        async void Delete(string url,ViewUser data)
        {
            //删除单条
